...
from mhr_api.models import EventTracking, MhrRegistration, registration_json_utils
...
def update_reg_report_batch_url(json_data: dict, batch_url: str) -> int:
    ...
    db.session.commit()
    if result:
        logger.debug(f"Updated {update_count} report registrations batch url to {batch_url}.")
    return update_count
...
def get_batch_location_report_data(start_ts: str = None, end_ts: str = None) -> dict:
    ...
    if results_json:
        logger.debug(f"Found {len(results_json)} NOC location registrations.")
    else:
        logger.debug("No NOC location registrations found within the timestamp range.")
    return results_json
...
def batch_location_report_empty(notify: bool, start_ts: str, end_ts: str):
    ...
    logger.info(message)
    if notify:
        logger.debug("Sending notify with no noc location registrations.")
...
def batch_location_report_response(raw_data, report_url: str, notify: bool):
    ...
    if notify:
        logger.debug(f"Sending noc location notify message with download link to {report_url}")
...
def batch_location_report_exists(batch_storage_url: str, notify: bool, return_link: bool):
    ...
    report_url: str = None
    raw_data = None
    logger.info(f"Fetching batch noc location registration report for: {batch_storage_url}.")
    if return_link:
        report_url = GoogleStorageService.get_document_link(
            batch_storage_url, DocumentTypes.BATCH_REGISTRATION, DEFAULT_DOWNLOAD_DAYS
        )
    else:
...
def save_batch_location_report(registrations, raw_data, return_link: bool) -> str:
    ...
    logger.info(f"Saving batch noc location registration report to: {batch_storage_url}.")
    if return_link:
        link = GoogleStorageService.save_document_link(
            batch_storage_url, raw_data, DocumentTypes.BATCH_REGISTRATION, DEFAULT_DOWNLOAD_DAYS
        )
    else:
...
def is_previous_location_doc_type(doc_type: str, json_data: dict) -> bool:
    ...
    ) and not json_data.get("location"):
        return False
...
def is_previous_owner_doc_type(doc_type: str, json_data: dict) -> bool:
    ...
    if doc_type in (
        ...
        MhrDocumentTypes.EXRE,
        MhrDocumentTypes.PUBA,
    ) and not json_data.get("ownerGroups"):
        return False
...
def set_batch_json_location(reg_json: dict, current_json: dict, doc_type: str, registration: MhrRegistration) -> dict:
    ...
    if (
        doc_type
        in (MhrDocumentTypes.REGC_CLIENT, MhrDocumentTypes.REGC_STAFF, MhrDocumentTypes.EXRE, MhrDocumentTypes.PUBA)
        and not registration.locations
    ):
        logger.debug(f"No location change: skipping previous location for doc type={doc_type}.")
    elif is_previous_location_doc_type(doc_type, reg_json) and current_json:
        logger.debug(f"Setting up previous location for doc type={doc_type}.")
...
def set_batch_json_owners(reg_json: dict, current_json: dict, doc_type: str, registration: MhrRegistration) -> dict:
    ...
    if (
        doc_type
        in (MhrDocumentTypes.REGC_CLIENT, MhrDocumentTypes.REGC_STAFF, MhrDocumentTypes.EXRE, MhrDocumentTypes.PUBA)
        and not registration.owner_groups
    ):
        logger.debug(f"No owner change: skipping previous owner groups for doc type={doc_type}.")
    elif is_previous_owner_doc_type(doc_type, reg_json) and current_json:
        logger.debug(f"Setting up previous owners for doc type={doc_type}.")
...
def get_batch_registration_data(start_ts: str = None, end_ts: str = None):
    ...
    if start_ts and end_ts:
        ...
        logger.debug("Using a default timestamp range of within the previous 7 days.")
    query = text(query_s)
    result = None
    if start_ts and end_ts:
        start: str = get_query_ts(start_ts)
        end: str = get_query_ts(end_ts)
        logger.debug(f"query parameters start={start} end={end}")
        result = db.session.execute(query, {"query_val1": start, "query_val2": end})
    else:
        result = db.session.execute(query)
    rows = result.fetchall()
    ...
    if results_json:
        logger.debug(f"Found {len(results_json)} batch registrations.")
    else:
        logger.debug("No batch registrations found within the timestamp range.")
    return results_json
