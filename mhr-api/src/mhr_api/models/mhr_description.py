...
class MhrDescription(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    def json(self) -> dict:  # pylint: disable=too-many-branches
        ...
        description = {
            ...
            "baseInformation": {"make": "", "model": "", "circa": False},
    ...
    def find_by_registration_id(cls, registration_id: int = None):
        ...
        if registration_id:
            descriptions = (
                db.session.query(MhrDescription)
                .filter(MhrDescription.registration_id == registration_id)
                .order_by(MhrDescription.id)
                .all()
            )
    ...
    def find_by_change_registration_id(cls, registration_id: int = None):
        ...
        if registration_id:
            description = (
                db.session.query(MhrDescription)
                .filter(MhrDescription.change_registration_id == registration_id)
                .one_or_none()
            )
        return description
    ...
    def create_from_json(json_data, registration_id: int = None):
        ...
        description: MhrDescription = MhrDescription(
            ...
            circa="N",
            manufacturer_name=json_data.get("manufacturer", "").strip().upper(),
