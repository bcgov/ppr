...
class MhrExtraRegistration(db.Model):
    ...
    def find_by_id(cls, extra_registration_id: int):
        ...
        return (
            db.session.query(MhrExtraRegistration)
            .filter(MhrExtraRegistration.id == extra_registration_id)
            .one_or_none()
        )
    ...
    def find_by_mhr_number(cls, mhr_number: str, account_id: str):
        ...
        if mhr_number and account_id:
            return (
                db.session.query(MhrExtraRegistration)
                .filter(MhrExtraRegistration.mhr_number == mhr_number, MhrExtraRegistration.account_id == account_id)
                .one_or_none()
            )
        return None
    ...
    def find_by_account_id(cls, account_id: str):
        ...
        if account_id:
            return db.session.query(MhrExtraRegistration).filter(MhrExtraRegistration.account_id == account_id).all()
        return None
    ...
    def find_mhr_numbers_by_account_id(cls, account_id: str):
        ...
        if account_id:
            registrations = (
                db.session.query(MhrExtraRegistration).filter(MhrExtraRegistration.account_id == account_id).all()
            )
            if registrations:
