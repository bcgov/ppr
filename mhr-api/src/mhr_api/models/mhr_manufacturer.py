...
class MhrManufacturer(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    submitting_party_id = db.mapped_column(
        "submitting_party_id", db.Integer, db.ForeignKey("mhr_parties.id"), nullable=False
    )
    ...
    def json(self) -> dict:
        ...
        owners.append(owner)
        group = {"groupId": 1, "type": "SOLE", "owners": owners}
    ...
    def find_by_id(cls, pkey: int = None):
        ...
        if pkey:
            try:
                ...
                logger.error("MhrManufacturerfind_by_id exception: " + str(db_exception))
    ...
    def find_by_registration_id(cls, reg_id: int):
        ...
        if reg_id:
            try:
                manufacturer = (
                    db.session.query(MhrManufacturer).filter(MhrManufacturer.registration_id == reg_id).one_or_none()
                )
            except Exception as db_exception:  # noqa: B902; return nicer error
                logger.error("MhrManufacturer.find_by_registration_id exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
        return manufacturer
    ...
    def find_by_account_id(cls, account_id: str):
        ...
        if account_id:
            try:
                manufacturer = (
                    db.session.query(MhrManufacturer).filter(MhrManufacturer.account_id == account_id).one_or_none()
                )
            except Exception as db_exception:  # noqa: B902; return nicer error
                logger.error("MhrManufacturer.find_by_account_id exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
        return manufacturer
    ...
    def create_manufacturer_from_json(json_data, account_id: str, user_id: str):
        ...
        registration: MhrRegistration = MhrRegistration(
            registration_type=MhrRegistrationTypes.MANUFACTURER,
