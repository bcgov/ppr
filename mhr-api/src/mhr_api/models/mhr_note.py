...
from mhr_api.models import MhrDocument
from mhr_api.models import utils as model_utils
from mhr_api.utils.base import BaseEnum
from mhr_api.utils.logging import logger

from .db import db
from .type_tables import MhrDocumentType, MhrDocumentTypes, MhrNoteStatusTypes, MhrPartyTypes
...
class MhrNote(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    effective_ts = db.mapped_column("effective_ts", db.DateTime, nullable=True, index=True)
    non_residential_reason = db.mapped_column(
        "non_residential_reason", PG_ENUM(NonResidentialReasonTypes, name="exnrreasontype"), nullable=True
    )
    non_residential_other = db.mapped_column("non_residential_other", db.String(125), nullable=True)

    # parent keys
    document_id = db.mapped_column(
        "document_id", db.Integer, db.ForeignKey("mhr_documents.id"), nullable=False, index=True
    )
    ...
    status_type = db.mapped_column(
        "status_type",
        PG_ENUM(MhrNoteStatusTypes, name="mhr_note_status_type"),
        db.ForeignKey("mhr_note_status_types.status_type"),
        nullable=False,
    )

    # Relationships - MhrRegistration
    registration = db.relationship(
        "MhrRegistration", foreign_keys=[registration_id], back_populates="notes", cascade="all, delete", uselist=False
    )
    ...
    def find_by_id(cls, pkey: int = None):
        ...
        if pkey:
            try:
                ...
                logger.error("MhrNote.find_by_id exception: " + str(db_exception))
    ...
    def find_by_registration_id(cls, registration_id: int):
        ...
        if registration_id:
            try:
                notes = (
                    db.session.query(MhrNote)
                    .filter(MhrNote.registration_id == registration_id)
                    .order_by(MhrNote.id)
                    .all()
                )
            except Exception as db_exception:  # noqa: B902; return nicer error
                logger.error("MhrNote.find_by_registration_id exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
        return notes
    ...
    def find_by_document_id(cls, document_id: int):
        ...
        if document_id:
            try:
                ...
                logger.error("MhrNote.find_by_document_id exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
        return note
    ...
    def find_by_change_registration_id(cls, registration_id: int = None):
        ...
        if registration_id:
            try:
                notes = (
                    db.session.query(MhrNote)
                    .filter(MhrNote.change_registration_id == registration_id)
                    .order_by(MhrNote.id)
                    .all()
                )
            except Exception as db_exception:  # noqa: B902; return nicer error
                logger.error("MhrNote.find_by_change_registration_id exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
        return notes

    @staticmethod
    def create_from_json(
        reg_json, registration_id: int, document_id: int, registration_ts, change_registration_id: int = None
    ):
