...
from .db import db
from .type_tables import MhrOwnerStatusTypes, MhrTenancyTypes
...
class MhrOwnerGroup(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    interest_numerator = db.mapped_column("interest_numerator", db.Integer, nullable=True)
    interest_denominator = db.mapped_column("interest_denominator", db.Integer, nullable=True)
    ...
    tenancy_type = db.mapped_column(
        "tenancy_type",
        PG_ENUM(MhrTenancyTypes, name="mhr_tenancy_type"),
        db.ForeignKey("mhr_tenancy_types.tenancy_type"),
        nullable=False,
    )
    ...
    def find_by_id(cls, pkey: int = None):
        ...
        if pkey:
            try:
                ...
                logger.error("MhrOwnerGroup.find_by_id exception: " + str(db_exception))
    ...
    def find_by_registration_id(cls, reg_id: int):
        ...
        if reg_id:
            try:
                groups = (
                    db.session.query(MhrOwnerGroup)
                    .filter(MhrOwnerGroup.registration_id == reg_id)
                    .order_by(MhrOwnerGroup.id)
                    .all()
                )
            except Exception as db_exception:  # noqa: B902; return nicer error
                logger.error("MhrOwnerGroup.find_by_registration_id exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
        return groups
    ...
    def find_by_change_registration_id(cls, reg_id: int):
        ...
        if reg_id:
            try:
                ...
                logger.error("MhrOwnerGroup.find_by_change_reg_id exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
        return groups
    ...
    def create_from_json(reg_json, registration_id: int):
        ...
        group: MhrOwnerGroup = MhrOwnerGroup(
