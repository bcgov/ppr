...
class MhrServiceAgreement(db.Model):
    ...
    def find_by_id(cls, agreement_id: int):
        ...
        if agreement_id:
            agreement = (
                db.session.query(MhrServiceAgreement).filter(MhrServiceAgreement.id == agreement_id).one_or_none()
            )
    ...
    def find_by_version(cls, version: str = None):
        ...
        if version:
            agreement = (
                db.session.query(MhrServiceAgreement).filter(MhrServiceAgreement.version == version).one_or_none()
            )
        return agreement
    ...
    def update_user_profile(json_data: dict, account_id: str, username: str) -> int:
        ...
        update_count: int = 0
        logger.debug(f"Update user_profiles.service_agreements account={account_id}, user={username}")
        ...
        db.session.commit()
        if result:
            logger.debug(f"Updated {update_count} user_profiles.service_agreements to {agreement}.")
        return update_count
    ...
    def get_agreement_profile(account_id: str, username: str) -> dict:
        ...
        try:
            ...
            logger.error("get_agreement_profile exception: " + str(db_exception))
        return agreement_json
