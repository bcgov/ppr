...
def get_history_json(registration: MhrRegistration, is_reg_view: bool) -> dict:
    ...
    logger.debug(f"{registration.mhr_number} get home history reg_view={is_reg_view}.")
    registration.staff = True
    registration.current_view = False
    registration.report_view = False
    history_json = {"mhrNumber": registration.mhr_number, "statusType": registration.status_type}
    if is_reg_view:
...
def get_reg_json(  # pylint: disable=too-many-branches; just 2 more.
    registration: MhrRegistration, base_reg: MhrRegistration
) -> dict:
...
def get_description_json(
    registration: MhrRegistration, base_reg: MhrRegistration, include_reg_summary: bool = True
) -> dict:
    ...
    if include_reg_summary:
        ...
        if (
            base_reg
            and base_reg.change_registrations
            and description_json.get("status") != MhrStatusTypes.ACTIVE
            and registration.descriptions[0].registration_id != registration.descriptions[0].change_registration_id
        ):
...
def get_location_json(registration: MhrRegistration, base_reg: MhrRegistration) -> dict:
    ...
    if (
        base_reg
        and base_reg.change_registrations
        and location_json.get("status") != MhrStatusTypes.ACTIVE
        and registration.locations[0].registration_id != registration.locations[0].change_registration_id
    ):
    ...
    for group in registration.owner_groups:
        ...
        for owner_json in group_json.get("owners"):
            ...
            if (
                base_reg
                and base_reg.change_registrations
                and group_json.get("status") not in (MhrOwnerStatusTypes.ACTIVE, MhrOwnerStatusTypes.EXEMPT)
                and group.registration_id != group.change_registration_id
            ):
