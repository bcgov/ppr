...
from mhr_api.models import FinancingStatement, MhrRegistration, search_utils
...
class SearchResult(db.Model):  # pylint: disable=too-many-instance-attributes
        ...
        for select in self.search_select:
            if "selected" not in select or select["selected"]:
                ...
                if not found:  # No duplicates.
                    ...
                    logger.debug(f"find_by_id for mhr num {mhr_num} start id={mh_id}")
                    record = MhrRegistration.find_by_id(mh_id, False, True)
                    logger.debug(f"find_by_id for mhr num {mhr_num} end")
                    record.staff = staff
                    result = record.registration_json
                    if select.get("includeLienInfo", False):
                        logger.info(f"Searching PPR for MHR num {mhr_num}.")
        ...
        if search_id and not limit_by_date:
            ...
            if (
                ...
                error_msg = model_utils.ERR_SEARCH_TOO_OLD.format(
                    code=ResourceErrorCodes.TOO_OLD_ERR.value, search_id=search_id, min_ts=min_ts
                )
    ...
    def validate_search_select(select_json, search_id: int):  # pylint: disable=unused-argument
        ...
        if not search_result:
            error_msg = model_utils.ERR_SEARCH_NOT_FOUND.format(
                code=ResourceErrorCodes.NOT_FOUND_ERR.value, search_id=search_id
            )
            ...
            error_msg = model_utils.ERR_SEARCH_COMPLETE.format(
                code=ResourceErrorCodes.DUPLICATE_ERR.value, search_id=search_id
            )
        else:
            ...
            if original_results:
                for match in select_json:
                    ...
                    if not exists:
                        ...
                        logger.info(
                            f"Search {search_id} invalid mhr number in search selection: " + match.get("mhrNumber")
                        )
                        break
            elif select_json:
                logger.info(f"Search {search_id} invalid mhr numbers submitted with NIL search results.")
    ...
    def search_ppr_by_mhr_number(mhr_number):
        ...
        logger.info(f"Search_ppr_by_mhr_number search value={mhr_number}.")
        rows = None
        try:
            ...
            logger.error("Search_ppr_by_mhr_number query exception: " + str(db_exception))
            raise DatabaseException(db_exception) from db_exception
        ...
        try:
            for row in rows:
                ...
                logger.info(f"Found financing id={financing_id}.")
                ...
                financing_json = {"matchType": "EXACT", "financingStatement": financing.json}
                results_json.append(financing_json)
        except Exception as db_exception:  # noqa: B902; return nicer error
            logger.error("Search_ppr_by_mhr_number build results error: " + str(db_exception))
            raise DatabaseException(db_exception) from db_exception

        logger.info("Search_ppr_by_mhr_number results length=" + str(len(results_json)))
        return results_json
