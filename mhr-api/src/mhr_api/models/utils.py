...
TO_DB_SEARCH_TYPE = {"OWNER_NAME": "MI", "ORGANIZATION_NAME": "MO", "MHR_NUMBER": "MM", "SERIAL_NUMBER": "MS"}
...
def get_serial_number_key(serial_num: str) -> str:
    ...
    # logger.debug(f'6: key_bytes={key_bytes}')
    # 7 Return the last 3 characters.
    start_pos = len(key_bytes) - 3
    last_3_bytes: bytes = key_bytes[start_pos:]
    logger.debug(f"key={key} last 3 bytes={last_3_bytes} hex={last_3_bytes.hex()}")
...
def is_historical(financing_statement, create: bool):
    ...
    if (
        ...
        and financing_statement.expire_date < _datetime.utcnow()
    ):
...
def date_elapsed(date_iso: str):
    ...
    now = now_ts()
    today_date = date(now.year, now.month, now.day)
    # logger.info('Comparing now ' + today_date.isoformat() + ' with expiry ' + test_date.isoformat())
