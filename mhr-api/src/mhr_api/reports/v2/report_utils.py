...
def get_report_files(  # pylint: disable=too-many-branches
    request_data: dict, report_type: str, mail: bool = False
) -> dict:
    ...
    if report_type in (
        ...
        if report_type in (
            ...
            elif report_type == ReportTypes.MHR_ADMIN_REGISTRATION and request_data["templateVars"].get(
                "documentType", ""
            ) in (MhrDocumentTypes.REGC_STAFF, MhrDocumentTypes.REGC_CLIENT):
                title_text = "REGISTRY CORRECTION"
            elif (
                report_type == ReportTypes.MHR_ADMIN_REGISTRATION
                and request_data["templateVars"].get("documentType", "") == MhrDocumentTypes.NCAN
                and request_data["templateVars"]["note"].get("cancelledDocumentDescription")
            ):
                title_text += " (" + request_data["templateVars"]["note"].get("cancelledDocumentDescription") + ")"
...
def update_toc_page_numbers(json_data, reg_pdf_data):
    ...
    if json_data["totalResultsSize"] > 0:
        ...
        page_index = 0
        logger.info(f" TOC totalPageCount={pagecount}")
