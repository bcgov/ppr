...
def get_registration_report(  # pylint: disable=too-many-return-statements,too-many-locals
    ...
    try:
        ...
        logger.error(f"An error occurred while generating the MHR registration report. {report_err}")
        return resource_utils.service_exception_response(
            "An error occurred while generating the MHR registration report."
        )
    except ReportDataException as report_data_err:
        logger.error(f"The report could not be generated due to invalid or missing data. {report_data_err}")
        return resource_utils.service_exception_response(
            "The report could not be generated due to invalid or missing data."
        )
    except StorageException as storage_err:
        logger.error(f"The report could not be stored or retrieved due to a storage service error. {storage_err}")
        return resource_utils.service_exception_response(
            "The report could not be stored or retrieved due to a storage service error."
        )
