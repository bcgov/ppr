...
from mhr_api.models import MhrExtraRegistration, MhrRegistration
...
bp = Blueprint(
    "OTHER_REGISTRATIONS1", __name__, url_prefix="/api/v1/other-registrations"  # pylint: disable=invalid-name
)
...
def get_other_registration(identifier: str):
    ...
    try:
        ...
        logger.debug(f"get_other_registration identifier={identifier}.")
        ...
        return resource_utils.db_exception_response(
            db_exception, account_id, "GET Registration Summary id=" + identifier
        )
...
def post_other_registration(mhr_number: str):
    ...
    try:
        ...
        logger.debug(f"post_other_registration mhr_number={mhr_number}.")
        # Quick check: must have an account ID.
        account_id = resource_utils.get_account_id(request)
        if account_id is None:
            return resource_utils.account_required_response()
        ...
        if extra_existing and extra_existing.removed_ind == MhrExtraRegistration.REMOVE_IND:
            logger.info(f"Restoring MHR# {mhr_number} for account {account_id}")
        ...
        logger.info(f"Adding another account MHR# {mhr_number} for account {account_id}")
        ...
        return resource_utils.db_exception_response(
            db_exception, account_id, "Add account extra registration id=" + mhr_number
        )
    except Exception as default_exception:  # noqa: B902; return nicer default error
        return resource_utils.default_exception_response(default_exception)
...
def delete_other_registration(mhr_number: str):
    ...
    try:
        ...
        logger.debug(f"delete_other_registration mhr_number={mhr_number}.")
        # Quick check: must have an account ID.
        account_id = resource_utils.get_account_id(request)
        if account_id is None:
            return resource_utils.account_required_response()
        ...
        if extra_registration and not extra_registration.removed_ind:
            logger.info(f"Removing another account MHR# {mhr_number} for account {account_id}")
            MhrExtraRegistration.delete(mhr_number, account_id)
        # Or mark user account's registration as removed.
        elif not extra_registration:
            logger.info(f"Marking MHR# {mhr_number} as removed for account {account_id}")
            extra_registration = MhrExtraRegistration(account_id=account_id, mhr_number=mhr_number)
        ...
        return resource_utils.db_exception_response(
            db_exception, account_id, "Delete account extra registration id=" + mhr_number
        )
    except Exception as default_exception:  # noqa: B902; return nicer default error
        return resource_utils.default_exception_response(default_exception)
