...
def get_account_qualified_supplier():
    ...
    try:
        ...
        logger.info(f"Getting qualified supplier information for account {account_id}.")
        ...
        logger.info(f"No qualified supplier info found for account {account_id}.")
...
def post_account_qualified_supplier():
    ...
    try:
        ...
        if not authorized(account_id, jwt):
            return resource_utils.unauthorized_error_response(account_id)
        logger.info(f"Creating qualified supplier  information for account {account_id}.")
        supplier: MhrQualifiedSupplier = MhrQualifiedSupplier.find_by_account_id(account_id)
        if supplier:
...
def delete_account_qualified_supplier():
    ...
    try:
        ...
        if not authorized(account_id, jwt):
            return resource_utils.unauthorized_error_response(account_id)
        logger.info(f"delete_account_qualified_supplier account_id={account_id}")
...
def put_account_qualified_supplier():
    ...
    try:
        ...
        if not supplier:
            logger.info(f"No qualified supplier info found for account {account_id}.")
            return resource_utils.not_found_error_response("qualified supplier information", account_id)
        logger.info(f"Updating qualified supplier information for account {account_id}.")
