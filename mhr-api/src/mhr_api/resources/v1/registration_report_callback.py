...
bp = Blueprint(
    "REGISTRATION_REPORT1", __name__, url_prefix="/api/v1/registration-report-callback"  # pylint: disable=invalid-name
)
...
def post_registration_report_callback(registration_id: str):  # pylint: disable=too-many-return-statements
    ...
    try:
        logger.info(f"Registration report callback starting id={registration_id}.")
...
def get_registration_callback_report(
    ...
    try:
        ...
        raw_data, status_code, headers = get_callback_pdf(
            report_info.report_data, registration.account_id, report_info.report_type, None, None
        )
