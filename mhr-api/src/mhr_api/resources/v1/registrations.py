...
from mhr_api.models import EventTracking, MhrManufacturer, MhrRegistration, batch_utils
...
bp = Blueprint("REGISTRATIONS1", __name__, url_prefix="/api/v1/registrations")  # pylint: disable=invalid-name
...
def get_account_registrations():
    ...
    try:
        ...
        # logger.debug(f'get_account_registrations account={account_id}.')
        ...
        params: AccountRegistrationParams = AccountRegistrationParams(
            account_id=account_id, collapse=collapse_param, sbc_staff=is_read_staff
        )
        ...
        return resource_utils.db_exception_response(
            db_exception, account_id, "GET account registrations id=" + account_id
        )
...
def post_registrations():  # pylint: disable=too-many-return-statements,too-many-branches
    ...
    try:
        ...
        if not authorized_role(jwt, REGISTER_MH):
            logger.error("User not staff or missing required role: " + REGISTER_MH)
            return resource_utils.unauthorized_error_response(account_id)
        ...
        if get_group(jwt) == MANUFACTURER_GROUP:
            logger.debug(f"Manufacturer request looking up info for account={account_id}.")
            manufacturer = MhrManufacturer.find_by_account_id(account_id)
            if not manufacturer:
                logger.info(f"No manufacturer info found for account id={account_id}")
        ...
        if (
            ...
            and request_json["location"].get("address")
            and not request_json["location"]["address"].get("street")
        ):
...
def get_registrations(mhr_number: str):  # pylint: disable=too-many-return-statements, too-many-branches
    ...
    try:
        logger.info(f"get_registrations mhr_number={mhr_number}")
        ...
        if resource_utils.is_pdf(request):
            ...
            logger.info(f"Fetching registration report for MHR# {mhr_number}.")
            return reg_utils.get_registration_report(
                registration, response_json, report_type, jwt.get_token_auth_header(), HTTPStatus.CREATED
            )
        ...
        return resource_utils.db_exception_response(db_exception, account_id, "GET MH registration id=" + mhr_number)
    except Exception as default_exception:  # noqa: B902; return nicer default error
        return resource_utils.default_exception_response(default_exception)
...
def get_registration_history(mhr_number: str):  # pylint: disable=too-many-return-statements; just 1 more.
    ...
    try:
        logger.info(f"get_registration_history mhr_number={mhr_number}")
        ...
        # Verify request JWT and account ID
        if not is_staff(jwt):
            logger.error(f"Account {account_id} user not staff: get registration history is staff only.")
            return resource_utils.unauthorized_error_response(account_id)
...
def get_batch_manufacturer_registrations():  # pylint: disable=too-many-return-statements
    ...
    try:
        logger.info("getting batch manufacturer registrations")
        ...
        if start_ts and end_ts:
            ...
            logger.debug(f"Using request timestamp range {start_ts} to {end_ts}")
        ...
        if status_code not in (HTTPStatus.OK, HTTPStatus.CREATED):
            logger.error("Batch manufacturer report merge call failed: " + raw_data.get_data(as_text=True))
...
def get_batch_registrations():
    ...
    try:
        ...
        if not authorized(account_id, jwt):
            return resource_utils.unauthorized_error_response(account_id)
        logger.info(f"getting batch registrations account id {account_id}.")
        start_ts: str = request.args.get(model_reg_utils.START_TS_PARAM, None)
        end_ts: str = request.args.get(model_reg_utils.END_TS_PARAM, None)
        if start_ts and end_ts:
            start_ts = resource_utils.remove_quotes(start_ts)
            end_ts = resource_utils.remove_quotes(end_ts)
            logger.debug(f"Batch registrations using request timestamp range {start_ts} to {end_ts}")
...
def post_batch_noc_locations():  # pylint: disable=too-many-return-statements
    ...
    try:
        logger.info("getting batch noc location registrations")
        # Authenticate with request api key
        if not resource_utils.valid_api_key(request):
        ...
        if registrations[0].get("batchStorageUrl"):  # Report already generated so fetch it.
            return batch_utils.batch_location_report_exists(
                registrations[0].get("batchStorageUrl"), notify, return_link
            )
        raw_data, status_code, headers = get_batch_noc_location_report(registrations)
        if status_code not in (HTTPStatus.OK, HTTPStatus.CREATED):
            logger.error("Batch noc location report merge call failed: " + raw_data.get_data(as_text=True))
            return raw_data, status_code, headers
...
def get_batch_manufacturer_report(registrations):
    ...
    for reg in registrations:
        raw_data, status_code, headers = get_callback_pdf(
            reg.get("reportData"), reg.get("accountId"), ReportTypes.MHR_REGISTRATION_STAFF, None, None
        )
        if status_code in (HTTPStatus.OK, HTTPStatus.CREATED):
            reports.append(raw_data)
        else:
            logger.error(str(reg.get("registrationId")) + " report api call failed: " + raw_data.get_data(as_text=True))
            return raw_data, status_code, headers
    return Report.batch_merge(reports)
...
def save_batch_manufacturer_report(registrations, raw_data, return_link: bool) -> str:
    ...
    logger.info(f"Saving batch manufacturer registration report to: {batch_storage_url}.")
    if return_link:
        link = GoogleStorageService.save_document_link(
            batch_storage_url, raw_data, DocumentTypes.BATCH_REGISTRATION, batch_utils.DEFAULT_DOWNLOAD_DAYS
        )
    else:
...
def batch_manufacturer_report_exists(batch_storage_url: str, notify: bool, return_link: bool):
    ...
    report_url: str = None
    raw_data = None
    logger.info(f"Fetching batch manufacturer registration report for: {batch_storage_url}.")
    if return_link:
        report_url = GoogleStorageService.get_document_link(
            batch_storage_url, DocumentTypes.BATCH_REGISTRATION, batch_utils.DEFAULT_DOWNLOAD_DAYS
        )
    else:
...
def batch_manufacturer_report_empty(notify: bool, start_ts: str, end_ts: str):
    ...
    logger.info(message)
    if notify:
        logger.debug("Sending notify with no registration report.")
        reg_utils.email_batch_man_report_staff(None)
    EventTracking.create(
        reg_utils.EVENT_KEY_BATCH_MAN_REG,
...
def batch_manufacturer_report_response(raw_data, report_url: str, notify: bool):
    ...
    if notify:
        logger.debug(f"Sending notify message with download link to {report_url}")
...
def get_change_permit(registration: MhrRegistration, account_id: str) -> bool:
    ...
    if registration.change_registrations:
        for reg in registration.change_registrations:
            if reg.registration_type in (
                MhrRegistrationTypes.PERMIT,
                MhrRegistrationTypes.PERMIT_EXTENSION,
                MhrRegistrationTypes.AMENDMENT,
            ):
                if reg.notes and reg.notes[0].status_type == MhrNoteStatusTypes.ACTIVE and account_id == reg.account_id:
                    can_edit = True
    return can_edit
