...
from mhr_api.models import MhrManufacturer, MhrQualifiedSupplier, MhrServiceAgreement
...
def get_service_agreements():
    ...
    try:
        ...
        logger.info(f"Getting service agreement information for account {account_id}.")
...
def get_agreement_versions(version: str):  # pylint: disable=too-many-return-statements
    ...
    try:
        logger.info(f"get_agreement_versions version={version}")
        if version is None:
        ...
        if resource_utils.is_pdf(request):
            ...
            logger.info(f"Fetching service agreement pdf {doc_name} from doc storage.")
...
def post_agreement_versions(version: str):  # pylint: disable=too-many-return-statements
    ...
    try:
        logger.info(f"post_agreement_versions version={version}")
        if version is None:
            return resource_utils.path_param_error_response("version")
        # Quick check: must have an account ID.
        account_id = resource_utils.get_account_id(request)
        ...
        return resource_utils.db_exception_response(
            db_exception, account_id, f"POST service agreement version={version}"
        )
    except BusinessException as exception:
        return resource_utils.business_exception_response(exception)
    except Exception as default_exception:  # noqa: B902; return nicer default error
...
def accept_agreement_account(supplier: MhrQualifiedSupplier, manufacturer: MhrManufacturer):
    ...
    if supplier and (supplier.terms_accepted is None or supplier.terms_accepted != "Y"):
        ...
        supplier.save()
        logger.debug(f"Setting supplier.terms_accepted for account_id={supplier.account_id}")
    elif manufacturer and (manufacturer.terms_accepted is None or manufacturer.terms_accepted != "Y"):
        manufacturer.terms_accepted = "Y"
        manufacturer.save()
        logger.debug(f"Setting manufacturer.terms_accepted for account_id={manufacturer.account_id}")
