...
ORDER_TEMPLATE = {
    "order": {"productType": "parcelInfo", "fileReference": "folio", "productOrderParameters": {"parcelIdentifier": ""}}
}
...
def pid_lookup(pid: str) -> dict:
    ...
    try:
        ...
        # logger.debug('LTSA PID lookup url=' + api_url)
        response = requests.request("post", api_url, params=None, json=data, headers=headers, timeout=30.0)
        # if response:
        #    logger.info('LTSA api response=' + response.text)
        if not response.ok:
            return None
        ...
        logger.error(f"LTSA PID lookup connection failure using svc:{api_url}", err)
    return None
