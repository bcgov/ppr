...
UPDATE_DOCUMENT_ID_REQUIRED = "Provide the update document ID in the request. "
UPDATE_DOCUMENT_ID_INVALID = "The update document ID is invalid. "
UPDATE_DOCUMENT_ID_STATUS = "The provided update document ID refers to an inactive note or registration. "
NRED_INVALID_TYPE = "NRED notices can only be submitted with TAXN-type documents. "
NCAN_DOCUMENT_ID_REQUIRED = "Provide the document ID for the cancellation update. "
NCAN_DOCUMENT_ID_INVALID = "The cancellation update document ID is incorrect or not recognized. "
NCAN_DOCUMENT_ID_STATUS = "The cancellation ID refers to a note that is no longer active. "
NCAN_NOT_ALLOWED = "A cancel notice cannot be submitted for the specified registration {doc_type}. "
LOCATION_REQUIRED = "Include a new location in the registration details. "
ADD_OWNERS_MISSING = "New owner details need to be added, corrected or amended when removing existing ones. "
DELETE_OWNERS_MISSING = "Specify which owners to remove when adding new ones. "


def validate_admin_reg(registration: MhrRegistration, json_data, staff: bool = True) -> str:
    """Perform all extra manufacturer admin registration data validation checks not covered by schema validation."""
    error_msg: str = ""
    try:
        logger.info(f"Validating admin registration staff={staff}")
        if staff:
            error_msg += validate_doc_id(json_data)  # Staff initially required for all document types.
        elif registration:
        ...
        error_msg += validator_utils.validate_registration_state(
            registration, staff, MhrRegistrationTypes.REG_STAFF_ADMIN, doc_type
        )
        ...
        elif doc_type and doc_type == MhrDocumentTypes.CANCEL_PERMIT:
            error_msg += validator_utils.validate_cancel_permit(registration)
        elif doc_type and doc_type in (
...
def validate_giving_notice(json_data, doc_type: str) -> str:
    ...
    if not json_data.get("note"):
        return error_msg
    if doc_type and not json_data["note"].get("givingNoticeParty") and doc_type == MhrDocumentTypes.NRED:
...
def validate_ncan(registration: MhrRegistration, json_data) -> str:
    ...
    cancel_type: str = None
    cancel_doc_id = (
        json_data.get("updateDocumentId") if json_data.get("updateDocumentId") else json_data.get("cancelDocumentId")
    )
    note = reg_utils.get_cancel_note(registration, cancel_doc_id)
    if note:
        status = note.status_type
