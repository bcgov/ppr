...
"""
from mhr_api.models import MhrNote, MhrRegistration
...
from mhr_api.utils.admin_validator import validate_ncan
...
EFFECTIVE_FUTURE = "The effective date and time cannot be set to a future date. "
EFFECTIVE_PAST = (
    "For the specified registration document type {doc_type}, the effective date and time must not be in the past. "
)
EFFECTIVE_NOT_ALLOWED = "Effective date and time is not allowed for the registration document type {doc_type}. "
EXPIRY_REQUIRED = "An expiry date and time is mandatory for the registration document type. "
EXPIRY_NOT_ALLOWED = "An expiry date and time is not permitted for the registration document type. "
EXPIRY_PAST = "The expiry date and time must be set to a future date for the registration document type. "
EXPIRY_BEFORE_CURRENT = "The new expiry date and time must be later than the current expiry date and time. "
...
def validate_note(registration: MhrRegistration, json_data, staff: bool = False, group_name: str = None) -> str:
    ...
    error_msg: str = ""
    try:
        logger.info(f"Validating unit note registration staff={staff}, group={group_name}")
        if not staff and registration:
            error_msg += validator_utils.validate_ppr_lien(
                registration.mhr_number, MhrRegistrationTypes.REG_NOTE, staff
            )
        ...
        error_msg += validator_utils.validate_registration_state(
            registration, staff, MhrRegistrationTypes.REG_NOTE, doc_type
        )
...
def validate_giving_notice(json_data, doc_type: str) -> str:
    ...
    if (
        doc_type
        and not json_data["note"].get("givingNoticeParty")
        and doc_type
        in (
