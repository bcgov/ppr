...
class BCMailFileTransferService:
    ...
    def transfer_verification_statement(self, binary_data, registration_id, party_id):
        ...
        try:
            ...
            msg = "Transfer verification statement failed: " + str(err)
            logger.error(msg)
            raise FileTransferException(msg) from err
    ...
    def get_filename(self, registration_id, party_id):
        ...
        filename = self.VERIFICATION_FILENAME.format(
            statement_key=reg_key, day=str(today_local.day), month=str(today_local.month), year=str(today_local.year)
        )
        return filename
