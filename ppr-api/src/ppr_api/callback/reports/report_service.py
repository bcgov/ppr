...
def get_search_report(search_id: str):
    ...
    try:
        ...
        raise ReportException("Search report generation failed for id=" + search_id) from err
...
def get_mail_verification_statement(registration: Registration, party: Party, account_name: str = None):
    ...
    logger.info(f"Mail verification report id={event_id}.")
    try:
        ...
        msg = f"Mail verification report generation failed for id={event_id}: " + str(err)
        # logger.error(msg)
        raise ReportException(msg) from err
