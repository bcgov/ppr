...
class ClientCodeRegistration(db.Model):
    ...
    @staticmethod
    def create_address_change_from_json(json_data: dict, user_id: str, code: ClientCode):
        """Create a client party code address change registration from dict/json."""
        reg: ClientCodeRegistration = ClientCodeRegistration(
            create_ts=model_utils.now_ts(),
            request_data=json_data,
            user_id=user_id,
            client_code_type=ClientCodeTypes.CHANGE_ADDRESS,
        )
        reg.client_code = code
        return reg

    @staticmethod
    def create_name_address_change_from_json(json_data: dict, user_id: str, code: ClientCode):
        """Create a client party code name and address change registration from dict/json."""
        reg: ClientCodeRegistration = ClientCodeRegistration(
            create_ts=model_utils.now_ts(),
            request_data=json_data,
            user_id=user_id,
            client_code_type=ClientCodeTypes.CHANGE_NAME_ADDRESS,
        )
        reg.client_code = code
        return reg
