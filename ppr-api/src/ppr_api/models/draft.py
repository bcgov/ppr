...
class Draft(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    document_number = db.mapped_column(
        "document_number", db.String(10), nullable=False, unique=True, default=db.func.get_draft_document_number()
    )
    ...
    def find_by_document_number(cls, document_number: str = None, allow_used: bool = False):
        ...
        if document_number:
            try:
                ...
                logger.error("DB find_by_document_number exception: " + str(db_exception))
                raise DatabaseException(db_exception) from db_exception
    ...
    def build_account_draft_query_params(params: AccountRegistrationParams) -> dict:
        ...
        query_params = {"query_account": params.account_id, "max_results_size": max_results_size}
        if params.registration_number:
