...
class SecuritiesActNotice(db.Model):
    ...
    securities_act_type = db.mapped_column(
        "securities_act_type",
        PG_ENUM(SecuritiesActTypes, name="securities_act_type"),
    ...
    registration = db.relationship(
        ...
        back_populates="securities_act_notices",
        cascade="all, delete",
        uselist=False,
    )
    sec_act_type = db.relationship(
        "SecuritiesActType",
        foreign_keys=[securities_act_type],
        back_populates="securities_act_notice",
        cascade="all, delete",
        uselist=False,
    )
    securities_act_orders = db.relationship(
        "SecuritiesActOrder", order_by="asc(SecuritiesActOrder.id)", back_populates="securities_act_notice"
    )
    ...
    def find_by_id(cls, sec_id: int):
        ...
        if sec_id:
            securities_act = (
                db.session.query(SecuritiesActNotice).filter(SecuritiesActNotice.id == sec_id).one_or_none()
            )
    ...
    def find_by_registration_id(cls, reg_id: int):
        ...
        if reg_id:
            securities_acts = (
                db.session.query(SecuritiesActNotice)
                .filter(SecuritiesActNotice.registration_id == reg_id)
                .order_by(SecuritiesActNotice.id)
                .all()
            )
    ...
    def create_from_json(json_data, registration_id: int):
        ...
        securities_act = SecuritiesActNotice(
            securities_act_type=json_data.get("securitiesActNoticeType"), registration_id=registration_id
        )
        ...
        if json_data.get("securitiesActOrders"):
            ...
            for order_json in json_data.get("securitiesActOrders"):
                securities_act_order: SecuritiesActOrder = SecuritiesActOrder.create_from_json(
                    order_json, registration_id, securities_act.id
                )
