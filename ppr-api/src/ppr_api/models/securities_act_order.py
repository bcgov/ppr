...
class SecuritiesActOrder(db.Model):
    ...
    securities_act_notice_id = db.mapped_column(
        "securities_act_notice_id", db.Integer, db.ForeignKey("securities_act_notices.id"), nullable=False, index=True
    )

    # Relationships - Registration
    securities_act_notice = db.relationship(
        "SecuritiesActNotice",
        foreign_keys=[securities_act_notice_id],
    ...
    def json(self) -> dict:
        ...
        order = {"orderId": self.id, "courtOrder": self.court_order_ind == "Y"}
        if self.court_name:
    ...
    def find_by_notice_id(cls, notice_id: int = None):
        ...
        if notice_id:
            orders = (
                db.session.query(SecuritiesActOrder)
                .filter(SecuritiesActOrder.securities_act_notice_id == notice_id)
                .order_by(SecuritiesActOrder.id)
                .all()
            )
        return orders
    ...
    def create_from_json(json_data, registration_id: int, notice_id: int):
        ...
        order: SecuritiesActOrder = SecuritiesActOrder(
            registration_id=registration_id, securities_act_notice_id=notice_id, court_order_ind="Y"
        )
