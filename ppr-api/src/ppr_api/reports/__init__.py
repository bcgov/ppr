...
def get_pdf(report_data, account_id, report_type=None, token=None):
    ...
    try:
        ...
        logger.error(f"Generate report failed for account {account_id}, type {report_type}: " + repr(err))
...
def get_report_api_payload(report_data, account_id, report_type, account_name):
    ...
    try:
        ...
        logger.error(f"Get report payload data failed for account {account_id}, type {report_type}: " + repr(err))
        raise BusinessException(error=DEFAULT_ERROR_MSG, status_code=HTTPStatus.INTERNAL_SERVER_ERROR) from err
...
def get_verification_mail(  # pylint: disable=too-many-locals
    ...
    try:
        logger.info(f"Not implemented: account id={account_id}, name={account_name}")
        logger.info(f"Not implemented: reg id={registration_id}")
        if report_data:
            logger.info(f"report data length={len(report_data)}")
        if token:
