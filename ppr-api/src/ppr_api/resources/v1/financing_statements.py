...
bp = Blueprint('FINANCING_STATEMENTS1',  # pylint: disable=invalid-name
               __name__, url_prefix='/api/v1/financing-statements')
...
def post_verification_callback():
    ...
    try:
        ...
        if party_id < 0:
            return fs_utils.callback_error(resource_utils.CallbackExceptionCodes.SETUP_ERR.value, registration_id,
        ...
        if event_count > current_app.config.get('EVENT_MAX_RETRIES'):
            return fs_utils.callback_error(resource_utils.CallbackExceptionCodes.MAX_RETRIES.value, registration_id,
        ...
        if not registration:
            return fs_utils.callback_error(resource_utils.CallbackExceptionCodes.UNKNOWN_ID.value, registration_id,
        ...
        return fs_utils.callback_error(resource_utils.CallbackExceptionCodes.REPORT_DATA_ERR.value,
                                       registration_id,
                                       HTTPStatus.INTERNAL_SERVER_ERROR, party_id,
                                       f'Party={party_id}. ' + str(report_data_err))
    except Exception as default_err:  # noqa: B902; return nicer default error
        return fs_utils.callback_error(resource_utils.CallbackExceptionCodes.DEFAULT.value,
                                       registration_id,
                                       HTTPStatus.INTERNAL_SERVER_ERROR, party_id,
...
def post_reg_report_callback(registration_id: int):
    ...
    try:
        ...
        if events:
            event_count = len(events)
        if event_count > current_app.config.get('EVENT_MAX_RETRIES'):
            return fs_utils.registration_callback_error(resource_utils.CallbackExceptionCodes.MAX_RETRIES.value,
                                                        registration_id,
        ...
        if not registration:
            return fs_utils.registration_callback_error(resource_utils.CallbackExceptionCodes.UNKNOWN_ID.value,
                                                        registration_id,
                                                        HTTPStatus.NOT_FOUND)
        if not registration.verification_report:
            return fs_utils.registration_callback_error(resource_utils.CallbackExceptionCodes.SETUP_ERR.value,
                                                        registration_id,
        ...
        return fs_utils.registration_callback_error(resource_utils.CallbackExceptionCodes.DEFAULT.value,
                                                    registration_id,
                                                    HTTPStatus.INTERNAL_SERVER_ERROR,
                                                    str(default_err))
