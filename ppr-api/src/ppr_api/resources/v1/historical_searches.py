...
from ppr_api.models import SearchRequest, SearchResult, search_historical
...
bp = Blueprint(
    "HISTORICAL_SEARCHES1", __name__, url_prefix="/api/v1/historical-searches"  # pylint: disable=invalid-name
)
...
@cross_origin(origin="*")
@jwt.requires_auth
def post_historical_searches():  # pylint: disable=too-many-locals,too-many-return-statements
    ...
    try:
        ...
        if not request_json.get("searchDateTime"):
            logger.error("searchDateTime required for historical searches")
        ...
        logger.debug(f"Historical search ts={search_ts}")
        max_reg_id: int = search_historical.get_search_historical_id(search_ts)
        logger.debug(f"Historical search max registration id={max_reg_id}")
        ...
        logger.debug("Executed search query, building detail results")
        ...
        logger.debug("Generating report with detail results")
        rep_data, status_code, headers = get_callback_pdf(
            results_data,
            "HIST_SEARCH",
        ...
        logger.debug("Saving report to doc storage and generating a link.")
        doc_name = model_utils.get_search_doc_storage_name(result.search)
        doc_name = doc_name.replace("search", "historical-search")
        rep_url = GoogleStorageService.save_document_link(
            doc_name, rep_data, DocumentTypes.SEARCH_RESULTS, STORAGE_LINK_LIFE
        )
        logger.info(f"Saved {doc_name} document storage link: {rep_url}")
