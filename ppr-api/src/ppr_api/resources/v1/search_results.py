...
def callback_error(code: str, search_id: str, status_code, message: str = None):
    ...
    if status_code != HTTPStatus.BAD_REQUEST and code not in (resource_utils.CallbackExceptionCodes.MAX_RETRIES.value,
...
def enqueue_notification(search_id: str):
    ...
    try:
        ...
        current_app.logger.error(f'Enqueue notification failed for id={search_id}: ' + str(err))
        EventTracking.create(search_id,
