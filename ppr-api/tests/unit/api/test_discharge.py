...
TEST_CREATE_DATA = [
    ...
    ('Valid SBC office role', STATEMENT_VALID, [PPR_ROLE, GOV_ACCOUNT_ROLE], HTTPStatus.CREATED, True, 'TEST0001'),
...
TEST_GET_STATEMENT2 = [
    ...
    ('Valid Request sbc staff', [PPR_ROLE, GOV_ACCOUNT_ROLE], HTTPStatus.OK, True, 'TEST0D14', 'TEST0014'),
...
def test_create_discharge_rl(session, client, jwt, desc, json_data, roles, status, after_cla, reg_num):
    """Assert that a post discharge registration on an RL type works as expected before and after the CLA transition."""
    headers = None
    # setup
