...
from ppr_api.models import Search, Registration
...
def test_search_reg_num_financing(session):
    ...
    json_data = {
        ...
        'clientReferenceId': 'T-SQ-RN-1'
    }
    query = Search.create_from_json(json_data, 'PS12345')
    query.search()
...
#    print(result)
    ...
    assert result['results'][0]
    assert result['results'][0]['baseRegistrationNumber'] == 'TEST0001'
    assert result['results'][0]['registrationNumber'] == 'TEST0007-AM'
    assert result['results'][0]['createDateTime']
    assert result['results'][0]['matchType']
    assert result['results'][0]['registrationType']
...
#    print(result)
    ...
    assert result['results'][0]
    assert result['results'][0]['baseRegistrationNumber'] == 'TEST0001'
    assert result['results'][0]['registrationNumber'] == 'TEST0008-DT'
    assert result['results'][0]['createDateTime']
    assert result['results'][0]['matchType']
    assert result['results'][0]['registrationType']
...
def test_search_no_account(session):
    ...
    json_data = {
        ...
        'clientReferenceId': 'T-SQ-RN-4'
    }
    query = Search.create_from_json(json_data, None)
    query.search()
...
def test_search_startDateTime_invalid(session, client, jwt):
    ...
    with pytest.raises(BusinessException) as bad_request_err:
        Search.validate_query(json_data)
...
def test_search_SN_invalid_criteria(session, client, jwt):
...
def test_search_ID_invalid_criteria(session, client, jwt):
...
def test_search_BD_invalid_criteria(session, client, jwt):
