...
describe('Mhr Client Corrections', async () => {
  ...
  it('Submitting Party step: renders initial step of Mhr Registry Corrections', async () => {
    expect(wrapper.exists()).toBe(true)
    expect(wrapper.find('#registration-correction-header').text()).toBe('Registry Correction - Client Error or Omission')
    expect(wrapper.findComponent(Stepper).exists()).toBe(true)
    expect(wrapper.findAll('.step').length).toBe(5)
  ...
  it('Review Confirm step: Displays Staff Payment component for REGC_CLIENT', async () => {
    goToStep(5)
    expect(wrapper.vm.$route.name).toBe(RouteNames.MHR_REVIEW_CONFIRM)
  ...
  it('Correction Payload: Includes Description', async () => {
    ...
    // Doc Type
    expect(mhrCorrection.documentType).toBeTruthy()
    expect(mhrCorrection.documentType).toBe(APIRegistrationTypes.MHR_CORRECTION_CLIENT)
